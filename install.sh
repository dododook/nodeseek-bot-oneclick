...
REPO="${REPO:-yaoguangting/nodeseek-bot:latest}"
...
# TTY-safe prompt: works with both interactive runs and "curl | bash"
prompt_from_tty() {
  local var="$1"; local prompt="$2"; local secret="${3:-0}"
  if [[ -n "${!var:-}" ]]; then return 0; fi
  if [ -t 0 ]; then
    if [[ "$secret" == "1" ]]; then read -rsp "$prompt: " "$var"; echo
    else read -rp  "$prompt: " "$var"
    fi
  elif [[ -r /dev/tty ]]; then
    if [[ "$secret" == "1" ]]; then read -rsp "$prompt: " "$var" </dev/tty; echo
    else read -rp  "$prompt: " "$var" </dev/tty
    fi
  else
    echo "ERROR: $var not provided and no TTY available. Please set it as an environment variable." >&2
    exit 1
  fi
}
...
prompt_from_tty TELEGRAM_BOT_TOKEN "Enter TELEGRAM_BOT_TOKEN" 1
prompt_from_tty ADMIN_IDS "Enter ADMIN_IDS (comma separated, e.g. 123,456)" 0
